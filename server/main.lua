...
RDX.RegisterServerCallback('rdx_billing:payBill', function(source, cb, billId)
	local xPlayer = RDX.GetPlayerFromId(source)
	...
	MySQL.Async.fetchAll('SELECT sender, target_type, target, amount FROM billing WHERE id = @id', {
		...
		if result[1] then
			...
			local xTarget = RDX.GetPlayerFromIdentifier(result[1].sender)
			...
			if result[1].target_type == 'player' then
					...
					if xPlayer.getMoney() >= amount then
						MySQL.Async.execute('DELETE FROM billing WHERE id = @id', {
							...
							if rowsChanged == 1 then
								...
								xPlayer.showNotification(_U('paid_invoice', RDX.Math.GroupDigits(amount)))
								xTarget.showNotification(_U('received_payment', RDX.Math.GroupDigits(amount)))
							end
				...
				TriggerEvent('rdx_addonaccount:getSharedAccount', result[1].target, function(account)
					if xPlayer.getMoney() >= amount then
						MySQL.Async.execute('DELETE FROM billing WHERE id = @id', {
							['@id'] = billId
						}, function(rowsChanged)
